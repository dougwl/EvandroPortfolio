...
commander_1.default.option("--config [path]", "Path to a spack.config.js file to use.");
// TODO: allow using ts. See: https://github.com/swc-project/swc/issues/841
commander_1.default.option("--mode <development | production | none>", "Mode to use");
commander_1.default.option('--context [path]', ` The base directory (absolute path!) for resolving the 'entry'`
    + ` option. If 'output.pathinfo' is set, the included pathinfo is shortened to this directory`, undefined, 'The current directory');
commander_1.default.option('--entry [list]', "List of entries", collect);
// commander.option('-W --watch', `Enter watch mode, which rebuilds on file change.`)
commander_1.default.option('--debug', `Switch loaders to debug mode`);
...
// Output options:
commander_1.default.option('-o --output', `The output path and file for compilation assets`);
commander_1.default.option('--output-path', `The output directory as **absolute path**`);
...
function parseSpackArgs(args) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        //
        const cmd = commander_1.default.parse(args);
        const opts = cmd.opts();
        const cliOptions = {
            // watch: !!opts.watch,
            debug: !!opts.debug,
        };
        const configOpts = yield spack_1.compileBundleOptions((_a = opts.config) !== null && _a !== void 0 ? _a : path.resolve('spack.config.js'));
        if (opts.entry) {
        ...
        return {
            cliOptions,
            spackOptions: Object.assign({}, configOpts),
        };
    });
}
