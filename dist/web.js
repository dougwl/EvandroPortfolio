    ...
    _createClass(FieldValidation, [
        {
            ...
            value: function CheckOn(event, nodeObj, message, param) {
                ...
                if (!(name in this.fieldsList)) {
                    ...
                    if (nodeObj.value != '') {
                        if (this.fieldsList[name].status == 'on') this.StopThis(name);
                        else ClosePopUp();
                        SetTimer(720);
                    } else {
            ...
            value: function IsValid(objName, message) {
                ...
                if (field.obj.value != '' && field.obj.checkValidity() == false) {
                    ...
                    if (document.activeElement.name == objName || document.activeElement.tagName == 'BODY' && this.lastInFocus == objName) {
                        if (isInViewport(field.obj) && field.obj.value != '') field.obj.reportValidity();
                ...
                var assignToEvents = (function(element) {
                    ...
                    if (element.type == type || type == '') {
                        if (element.tagName == 'INPUT' && element.type != 'hidden' && !element.id.includes('botcheck')) {
                            if (message == '') {
                                if (element.hasAttribute('tooltip')) message = element.getAttribute('tooltip');
                                else console.warn('No message defined, using default tooltip.');
                            }
                            if (!element.hasAttribute('inputType')) element.addEventListener('input', (function(evt) {
                                this.CheckOn(evt, element, message, tooltipTime != '' ? tooltipTime : undefined);
                            }).bind(this));
                            else if (element.getAttribute('inputType') == 'calendar') element.addEventListener('blur', (function(evt) {
                                this.CheckOn(evt, element, message, tooltipTime != '' ? tooltipTime : undefined);
                            }).bind(this));
                            element.form.addEventListener('submit', function(ev) {
                                if (element.value == '') {
                                    element.setAttribute('required', '');
                                    element.setCustomValidity('Este campo \xe9 obrigat\xf3rio!');
                                    ev.stopPropagation();
                                    ev.preventDefault();
                    ...
                    } else throw console.error("Type is invalid.");
                }).bind(this);
                if (nodeList != undefined || nodeList != null) inputHandler = nodeList;
                else throw console.error('Node list is empty!');
                if (inputHandler instanceof NodeList && inputHandler.length > 0) {
                    var mismatchCount = 0;
                    for(var field in inputHandler)if (inputHandler.hasOwnProperty(field)) {
                        var element = inputHandler[field];
                        if (element.tagName == 'INPUT') assignToEvents(element);
                        else mismatchCount += 1;
                    }
                    if (mismatchCount == inputHandler.length) throw console.warn("No inputs found!");
                } else if (inputHandler.tagName == 'INPUT') assignToEvents(inputHandler);
                else throw console.warn("No inputs found!");
            }
        }
    ]);
...
var CustomPlaceHolder = function() {
    ...
    function CustomPlaceHolder(param) {
        var _Default = param.Default, Default = _Default === void 0 ? '' : _Default, _Custom = param.Custom, Custom = _Custom === void 0 ? '' : _Custom, Target = param.Target, _StartWithDefault = param.StartWithDefault, StartWithDefault = _StartWithDefault === void 0 ? true : _StartWithDefault;
    ...
    _createClass(CustomPlaceHolder, [
        {
            ...
            value: function OnFocus(param) {
                ...
                if (ReturnToDefault == true) this.Node.onblur = (function() {
    ...
    _createClass(VideoController, [
        ...
        },
        {
            ...
            value: function StopAll() {
                ...
                try {
                    for(var _iterator = this.players[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true){
                        var player = _step.value;
                        if (player.paused || player.playing) player.stop();
                    }
                } catch (err) {
                    _didIteratorError = true;
                    _iteratorError = err;
        ...
        },
        {
            ...
            value: function DisablePlayOnClick(toggle, param) {
                ...
                var _iteratorNormalCompletion = true, _didIteratorError = false, _iteratorError = undefined;
                if (param1.all && !(param1.player != null)) try {
                    for(var _iterator = this.players[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true){
                        var pl = _step.value;
                ...
                else if (param1.player != null) param1.player.config.clickToPlay = !toggle;
            }
        }
    ]);
...
var ScrollObserver = function() {
     ...
     */ key: "_defaultCallback",
        ...
        {
            key: "On",
            value: function On(param, callBack) {
                var event = param === void 0 ? '' : param;
                if (callBack !== undefined) {
                    if (event === 'OnScrollMove') {
                        ...
                        this._numberOfSignedEvents += 1;
                    } else return console.error("Event not supported");
                } else console.error('Callback is undefined.');
            }
        },
        {
            key: "Off",
            value: function Off(param, callBack) {
                var event = param === void 0 ? '' : param;
                if (callBack !== undefined && this._numberOfSignedEvents > 0) {
                    if (event === 'OnScrollMove') {
                        ...
                        this._numberOfSignedEvents -= 1;
                    } else return console.error("Only event supported is OnScrollMove");
                } else console.error('Callback is undefined.');
            }
        }
    ]);
    ...
    _createClass(Sort, null, [
        {
            ...
            value: function quickSort(originalArray) {
                if (originalArray.length <= 1) return originalArray;
                    ...
                    for(var i = 0; i < length; i++)if (originalArray[i] <= pivot) leftSide.push(originalArray[i]);
                    else rightSide.push(originalArray[i]);
                     //END FOR   
    ...
    _createClass(WatchScrollPosition, [
                        ...
                        if (ExcludedIDs.length > 0) {
                            ExcludedIDs = ExcludedIDs.filter(function(val) {
                                nodes = Array.from(nodes).filter(function(node) {
                                    return node.id == val ? (exclude = true, false) : (exclude = false, true);
                                });
                                return !exclude;
                            });
                            elements[tag] = nodes;
                        } else elements[tag] = Array.from(nodes);
        ...
        },
        {
            ...
            value: function Watch(param) {
                var tmp = param.State, State = tmp === void 0 ? true : tmp, Callback = param.Callback, scrollObserver = param.scrollObserver;
                if (State) {
                    ...
                    if (scrollObserver === undefined) {
                        if (this.ScrollObserver === undefined) {
                            if (ScrollObserver.ActiveObservers.length > 0) this.ScrollObserver = ScrollObserver.ActiveObservers[0];
                            else this.ScrollObserver = new ScrollObserver();
                        }
                    } else if (this.ScrollObserver === undefined) this.ScrollObserver = ScrollObserver;
                    this.ScrollObserver.On('OnScrollMove', (function() {
                        for(var i = 0; i < this.Positions.length; i++){
                            ...
                            if (window.pageYOffset >= position && !(window.pageYOffset >= this.Positions[i + 1]) && position != this._lastPosition) {
                                ...
                                if (Callback !== undefined) {
                                    for(var subscriber in this.Subscribers)this.Subscribers[subscriber](position, this.Nodes);
                                    break;
                                }
                            }
                        }
                    }).bind(this));
                } else for(var method in this.Subscribers)if (method == Callback.name) {
        ...
        },
        {
            ...
            value: function CurrentSection() {
                ...
                for(var i = 0; i < this.Positions.length; i++){
                    ...
                    if (windowPosition >= sectionPosition && !(windowPosition >= this.Positions[i + 1])) return this.Nodes[this.Positions[i]];
                    else if (windowPosition < this.Positions[0]) return this.Nodes[this.Positions[0]];
                }
            }
        }
    ]);
...
HideNavbar.On('OnScrollMove', function(val) {
    if (val.detail.Up) Navbar.style = "opacity: 1;";
...
var ScrollIntoView = _asyncToGenerator(regeneratorRuntime.mark(function _callee(element) {
    var node;
    return regeneratorRuntime.wrap(function _callee$(_ctx) {
        while(1)switch(_ctx.prev = _ctx.next){
            case 0:
        ...
        resetTransitions: function resetTransitions() {
            ...
            try {
                for(var _iterator = this.instance.cells[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true){
                    ...
                    if (canvas.classList.contains("video-carousel--fade-complete")) canvas.classList.remove("video-carousel--fade-complete");
                }
            } catch (err) {
                _didIteratorError = true;
                _iteratorError = err;
...
/* document.body.addEventListener('load', LoadIframes()); */ var EnableOverlay = new function() {
    ...
    playButton.addEventListener('click', function() {
        ...
        if (playerController === undefined) LoadIframes();
        ...
        overlay.addEventListener('click', function CloseOverlay(ev) {
            if (ev.target != this && !ev.target.classList.contains('icon-line-cross') && !ev.target.classList.contains('video-overlay-close')) return;
            changeOverlayStatus();
...
var anchorList = document.getElementsByClassName('nav-link py-3');
var _iteratorNormalCompletion = true, _didIteratorError = false, _iteratorError = undefined;
try {
    for(var _iterator = anchorList[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true){
        var anchor = _step.value;
        anchor.onclick = function() {
            var tabPanel = document.getElementsByClassName('tab-pane')[0];
            var cityName = anchor.getAttribute('aria-controls');
            if (tabPanel.id != cityName) {
                var form = tabPanel.getElementsByTagName('form')[0];
                ...
                setTimeout(function() {
                    var cityField = form.elements.namedItem('contato-form-city');
    ...
    if (!el.currentTarget.classList.contains('--active')) {
        ...
        for(var button in perfilButtons){
            ...
            if (button.id != activeButton.id) {
                ...
                if (button.classList.contains('--inactive')) button.classList.remove('--inactive');
                if (!button.classList.contains('--active')) button.classList.add('--active');
            }
        }
        if (!document.body.classList.contains('--obscured')) {
            if (document.documentElement.clientWidth <= 834) document.body.classList.add('--obscured');
        }
    } else {
        ...
        if (document.body.classList.contains('--obscured')) document.body.classList.remove('--obscured');
    }
    innerVisualHeight();
};
for(var button in perfilButtons){
